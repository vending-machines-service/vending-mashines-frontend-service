...
private List<MachineProductSensorJPA> toMachineProdSens(Map<Integer, Integer> productSensor, 
		MachineJPA machineJpa) {
	...
	for(Map.Entry<Integer, Integer> map : productSensor.entrySet()) {
		...
		MachineProductSensorJPA sensorProduct = new MachineProductSensorJPA(machineJpa.machineId, 
				map.getKey(), map.getValue(), prod.getName() );
...
private MachineStateDTO convertCurrentStateToDTO(MachineStateCurrentDTO stateMachine) {
	...
	for(MachineProductSensorJPA sens : sensorProd) {
		...
		if(sensorValue.get(sens.getSensorId()) == null) state = MachineStateEnum.DOWNTIME;
	}
